...
/// read/write (sys_top `ram1` — ring fetcher, blit engine, scanout
/// writer), 2/3 = f2h_sdram1 (`ram2` — texture unit), 4/5 =
/// f2h_sdram2 (`vbuf` — the ASCAL HDMI framebuffer scanout), 6-9 =
/// L3/MPU. u-boot leaves the whole register at 0, i.e. round-robin.
///
/// The menu core's masters put sustained multi-burst traffic on
/// ram1/ram2; under load the round-robin arbiter starves vbuf's
/// real-time reads and the scaler re-emits its last 128-byte burst —
/// on HDMI that is a full-width band of 32-pixel-period vertical
/// stripes (one repeated burst) across whatever rows were scanned
/// during the stall. Making vbuf the highest-priority port removes
/// the artifact; the bulk masters simply absorb the extra latency.
...
impl Device {
    ...
    /// Unix epoch stamped at the moment the loaded RBF was
    /// synthesized. Generated by `just _gen-build-id` and embedded
    /// via the `BUILD_ID` Verilog macro. Reading 0 typically means
    /// the bitstream predates this register; reading 0xFFFFFFFF
    /// means the macro defaulted to all-ones (build_id.svh missing
    /// at synthesis time).
    #[inline]
    pub fn build_id(&self) -> u32 {
        self.regs.read32(registers::BUILD_ID)
    }
    ...
    /// Allocate a layer source render-target — the host's per-frame
    /// pixel target in compositor-v2 (COMPOSITOR_V2.md §4.1). Identical
    /// allocation semantics to [`Self::create_render_target`]; the
    /// distinct name documents intent at the call site.
    ///
    /// The spec calls out an eventual split between the texture pool
    /// (small atlases / glyphs) and a separate layer-RT pool (large,
    /// full-screen targets). v1 leaves them unified — a layer RT and
    /// a regular RT both live in the same bump-allocated DDR3 region.
    /// When pool fragmentation becomes a concern we can swap the
    /// underlying allocator without changing this API.
    #[inline]
    pub fn create_layer_rt(
        &mut self,
        width: u16,
        height: u16,
    ...
    /// Diagnostic: read `len` bytes from the ring buffer starting at
    /// `offset` (relative to RING_BASE). Used to sanity-check what
    /// the host actually wrote into the ring vs what the FPGA is
    /// fetching — when a FENCE timeout includes a bad-opcode halt
    /// this reveals whether the corruption is on the host side
    /// (host wrote 0xFF at that offset) or somewhere on the bus
    /// (FPGA's read returned data different from what we wrote).
    pub fn read_ring_bytes(&mut self, offset: u32, len: usize) -> Vec<u8> {
        let mut out = vec![0u8; len];
        let cap = self.cfg.ring_size as u32;
        let base = self.ring_map.as_mut_ptr();
        for i in 0..len {
            let o = (offset.wrapping_add(i as u32)) & (cap - 1);
            // SAFETY: the ring mmap covers `cfg.ring_size` bytes and
            // o is masked to that range. The ring mapping is read-write,
            // so volatile reads are valid.
            unsafe {
                out[i] = core::ptr::read_volatile(base.add(o as usize));
            }
        }
        out
    }

    /// Diagnostic: read `len` bytes from the layer-table LW_H2F BRAM
    /// at `offset` (relative to its base, 0xFF22_0000). Bank A is at
    /// 0x0000..0x1FFF, bank B at 0x2000..0x3FFF. Used to verify the
    /// host's writes via LW_H2F actually land in the BRAM that the
    /// compositor reads from.
    pub fn read_layer_bram_bytes(&self, offset: u32, len: usize) -> Option<Vec<u8>> {
        let mut map = DevMemMap::create(LAYER_BRAM_PHYS_ADDR + offset, len).ok()?;
        let mut out = vec![0u8; len];
        let base = map.as_mut_ptr();
        for i in 0..len {
            unsafe {
                out[i] = core::ptr::read_volatile(base.add(i));
            }
        }
        Some(out)
    }

    /// Diagnostic: open a fresh mmap covering `map_len` bytes from
    /// `fb_base` and read `len` bytes at `fb_offset` within that
    /// mmap. Used to verify whether the compositor's scanout writer
    /// is actually depositing non-zero pixels at the FB the v2 mux
    /// points MISTER_FB at — distinct from `read_tex_pool_bytes`
    /// which checks the LAYER SOURCE side. `/dev/mem` mmap requires
    /// page-aligned base and length so the caller passes a base
    /// that's a multiple of 4 KiB and a `map_len` big enough to
    /// cover the highest offset they intend to read; the function
    /// handles the read internally.
    pub fn read_fb_bytes(
        &self,
        fb_base: u32,
        fb_offset: u32,
        len: usize,
    ) -> Option<Vec<u8>> {
        // Round up to a page-aligned mmap covering at least `fb_offset+len`.
        let total = (fb_offset as usize) + len;
        let page = 4096usize;
        let map_len = ((total + page - 1) / page) * page;
        let mut map = DevMemMap::create(fb_base, map_len).ok()?;
        let mut out = vec![0u8; len];
        let base = map.as_mut_ptr();
        for i in 0..len {
            unsafe {
                out[i] = core::ptr::read_volatile(base.add(fb_offset as usize + i));
            }
        }
        Some(out)
    }

    /// Diagnostic: read `len` bytes from the texture pool at
    /// `pool_offset` (relative to TEX_POOL base, i.e.
    /// `base_phys_addr + TEX_POOL_OFFSET`). Used to verify that the
    /// host's blit-engine writes are actually reaching the texture
    /// pool in DDR3 — in particular, "is the L0 layer source RT at
    /// 0x32000000 holding non-zero painted pixels, or did the blits
    /// silently land somewhere else?" Reads via the texture pool
    /// mmap, which is the same /dev/mem region the FPGA reads from,
    /// so this is the most direct possible coherence check.
    pub fn read_tex_pool_bytes(&mut self, pool_offset: u32, len: usize) -> Option<Vec<u8>> {
        let pool = self.tex_pool_map.as_mut()?;
        let cap = pool.len();
        if (pool_offset as usize) + len > cap {
            return None;
        }
        let mut out = vec![0u8; len];
        ...
        for i in 0..len {
            unsafe {
                out[i] = core::ptr::read_volatile(base.add(pool_offset as usize + i));
            }
        }
        Some(out)
    }
