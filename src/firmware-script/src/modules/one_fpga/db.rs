...
fn load_path_(path: JsString, context: &mut Context) -> JsResult<JsObject> {
    JsDb::from_data(
        JsDb::load_file(&PathBuf::from(&path.to_std_string_lossy()))?,
        context,
    )
}
...
pub fn create_module(context: &mut Context) -> JsResult<(JsString, Module)> {
    let module = [
        ...
        (
            js_string!("loadPath"),
            load_path_.into_js_function_copied(context),
        ),
