...
export abstract class CoreCommandImpl<T = undefined> extends CommandImpl<T> {
  execute_(core: core.OneFpgaCore | undefined, v: T) {
    if (core) {
  ...
  abstract execute(core: core.OneFpgaCore, v: T): void | Promise<void>;
}
...
class Shortcuts {
  ...
  public static async create<Json>(
    ...
    if (!command.validate(meta)) {
      throw new Error(`${JSON.stringify(meta)} does not match the command schema.`);
    }
...
export class Command<T = unknown> {
  ...
  public async execute(core: core.OneFpgaCore | undefined, meta: T) {
