...
import { launchGame } from '@/services/utils';

const PAGE_SIZE = 100;

const SORT_LABEL = {
  [db.games.GameSortOrder.NameAsc]: 'Name (A-Z)',
  [db.games.GameSortOrder.NameDesc]: 'Name (Z-A)',
  [db.games.GameSortOrder.SystemAsc]: 'System (A-Z)',
  [db.games.GameSortOrder.LastPlayed]: 'Last Played',
  [db.games.GameSortOrder.Favorites]: 'Favorites',
};
...
export async function pickGame(
...
): Promise<db.games.ExtendedGamesRow | null> {
  const userSettings = await settings.UserSettings.forLoggedInUser();
  ...
  async function buildItems(): Promise<osd.TextMenuItem<db.games.ExtendedGamesRow | string>[]> {
    const { total, games } = await db.games.listExtended({
      system: options.system,
      includeUnplayed,
      sort: Object.keys(SORT_LABEL)[currentSort] as db.games.GameSortOrder,
      limit: PAGE_SIZE,
    });

    const gameSet: Map<String, db.games.ExtendedGamesRow[]> = games.reduce((acc, game) => {
  ...
  let selected: string | db.games.ExtendedGamesRow = 'next';

  while (selected === 'next') {
    const items = await buildItems();
    selected = await osd.textMenu<string | db.games.ExtendedGamesRow>({
      title: options.title ?? '',
      back: 'back',
      ...
      sort: async () => {
        ...
        index = 0;
        const order = Object.keys(SORT_LABEL)[currentSort] as db.games.GameSortOrder;
...
async function showGameDetailsMenu(
  name: string,
  gameArray: db.games.ExtendedGamesRow[],
): Promise<db.games.ExtendedGamesRow | null> {
...
): Promise<db.games.ExtendedGamesRow | false | number> {
  const shortcuts =
    db.Commands.get(StartGameCommand)?.shortcutsWithMeta.filter(s => {
      return gameArray.some(ga => ga.id === s.meta.gameId);
    }) ?? [];

  const result = await osd.textMenu<db.games.ExtendedGamesRow | false | number>({
    title: name,
    back: false,
    highlighted,
    items: [
      {
        ...
        select: async item => {
          await db.games.setFavorite(gameArray[0], !gameArray[0].favorite);
      ...
      ...(shortcuts.length > 0
        ? [
            ...
            ...shortcuts.map((s, i) => ({
              label: ` ${s.shortcut}`,
              select: async () => {
                const command = await db.Commands.get(StartGameCommand);
                if (command) {
        ...
        : []),
      {
        ...
        select: async () => {
          ...
          const command = db.Commands.get(StartGameCommand);
          if (command) {
            const maybeCommand = await db.Commands.find(newShortcut);
            if (maybeCommand) {
